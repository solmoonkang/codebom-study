...
public class 폰켓몬 {
    ...
    public int solution(int[] nums) {
        // 선택할 수 있는 폰켓몬의 최대 수이다. 즉, 전체 폰켓몬의 수를 2로 나눈 값이다.
        int maxSelection = nums.length / 2;

        // 중복되는 폰켓몬 번호를 제거하기 위해서 HashSet을 사용한다.
        HashSet<Integer> pokemons = new HashSet<>();

        // 중복되는 폰켓몬은 HashSet에 추가되지 않는다.
        for (int num : nums) {
            pokemons.add(num);
        }

        // 만약, 폰켓몬 수가 폰켓몬 최대 수보다 크다면 maxSelection을 반환하고 아니면 pokemons의 크기를 반환한다.
        // 즉, 중복되지 않은 폰켓몬의 수를 반환한다.
        if (pokemons.size() > maxSelection) {
            return maxSelection;
        }
    ...
    public static void main(String[] args) {
        폰켓몬 phoneketmon = new 폰켓몬();
        int[] firstNums = {3, 1, 2, 3};
        System.out.println("Result: " + phoneketmon.solution(firstNums));

        int[] secondNums = {3, 3, 3, 2, 2, 4};
        System.out.println("Result: " + phoneketmon.solution(secondNums));

        int[] thirdNums = {3, 3, 3, 2, 2, 2};
        System.out.println("Result: " + phoneketmon.solution(thirdNums));
    }
}
